...
local this_os = "baremetal" -- ffi.os

if this_os=='bare_metal' then
    ...
    function new(size)
        return checkh(ffi.C.malloc(size))
    end


    function free(addr, size)
        return ffi.C.free(addr)
    end
...
elseif this_os == 'Linux' or this_os == 'BSD' or this_os == 'OSX' then
